    ...
    legend
        ...
        function updateChart(event) {
            ...
            if (event.selection === null) {
                ...
                xScale.domain(ageData.map(d => d.age.toString()));
            } else {
                var eachBand = xScale.step();
                var index = Math.floor((event.selection[0] / eachBand));
                var val = xScale.domain()[index];

                var eachBand2 = xScale.step();
                var index2 = Math.floor((event.selection[1] / eachBand2));
                var val2 = xScale.domain()[index2];

                console.log(xScale.domain());
                console.log(val);
                console.log(val2);

                xScale.domain(ageData.slice(val, val2).map(d => d.age.toString()));
                console.log(xScale.domain());
                ...
                // Remove:
                g.selectAll(".male-bars").remove();
                g.selectAll(".female-bars").remove();
                // Step 6: Add bars to chart based on age data
                g.selectAll(".male-bars")
                    .data(ageMaleData.slice(val, val2))
                    .enter()
                    .append("rect")
                    .classed("male-bars", true)
                    .attr("x", function(d) { console.log(xScale(1)); return xScale(d.age.toString()); })
                    .attr("y", height)
                    .attr("width", xScale.bandwidth())
                    .attr("height", 0) // initial height as 0
                    .attr("fill", "steelblue")
                ...
                g.selectAll(".female-bars")
                    .data(ageFemaleData.slice(val, val2))
                    .enter()
                    .append("rect")
                    .classed("female-bars", true)
                    .attr("x", function(d) { return xScale(d.age.toString()); })
